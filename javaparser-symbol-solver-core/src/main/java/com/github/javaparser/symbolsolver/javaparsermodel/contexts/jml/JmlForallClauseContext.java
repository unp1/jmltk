 ...
 * @version 1 (08.07.26)
 */
public class JmlForallClauseContext extends AbstractJavaParserContext<JmlForallClause> {
    public JmlForallClauseContext(JmlForallClause wrappedNode, TypeSolver typeSolver) {
        super(wrappedNode, typeSolver);
    }

    @Override
    public List<Parameter> parametersExposedToChild(Node child) {
        return new ArrayList<>(wrappedNode.getBoundedVariables());
    }

    @Override
    public SymbolReference<? extends ResolvedValueDeclaration> solveSymbol(String name) {
        for (Parameter variable : wrappedNode.getBoundedVariables()) {
